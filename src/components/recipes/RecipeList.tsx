        ...
        <div className="flex items-center justify-between py-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="p-1.5 rounded-lg relative transition-all text-stone-400 hover:text-orange-500 active:bg-orange-100 active:text-orange-600 active:ring-1 active:ring-orange-300 dark:text-stone-500 dark:hover:text-orange-400 dark:active:bg-orange-950 dark:active:text-orange-400 dark:active:ring-orange-700"
              title="Add recipe"
            >
    ...
    >
      ...
      <div className="flex flex-1 flex-col justify-between p-3">
        ...
        <div className="flex items-center gap-3 mt-1.5 text-xs text-stone-500 dark:text-stone-400">
          ...
          {(() => {
            const ratingValue = recipe.avgRating ?? recipe.sourceRating;
            if (!ratingValue) return null;
            const isSource = !recipe.avgRating && !!recipe.sourceRating;
            return (
              <span
                className="flex items-center gap-1 text-amber-500"
                title={isSource ? `Source rating${recipe.sourceRatingCount ? ` (${recipe.sourceRatingCount})` : ""}` : "Our rating"}
              >
                <svg className={classNames("w-3.5 h-3.5", isSource ? "fill-none stroke-amber-400" : "fill-amber-400")} viewBox="0 0 24 24" strokeWidth={2}><path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg>
                {ratingValue}
              </span>
            );
          })()}
